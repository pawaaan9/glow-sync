...
interface NavItem {
  ...
  exact?: boolean;
  badge?: "pendingRequests" | "awaitingStaffAcceptance";
}
...
/** Salon logo + name where GlowSync's own mark sits in the other panels. */
function SidebarBrand() {
  const { me } = useAuth();
  const salon = me?.salon;

  return (
    <Link href="/salon-owner/dashboard" className="flex items-center gap-3 px-2">
      {salon?.logoUrl ? (
      ...
      <span className="flex min-w-0 flex-col leading-tight">
        <span className="font-display truncate text-base text-white">{salon?.name ?? "GlowSync"}</span>
        <span className="eyebrow text-rose-300/80">Salon Owner</span>
      </span>
    </Link>
  );
}
...
function SidebarAccount() {
  ...
  const name = me?.user.fullName ?? "Owner";
